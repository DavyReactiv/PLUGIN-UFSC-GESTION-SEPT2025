...
class UFSC_SQL_Admin
{
    ...
    public static function handle_save_licence()
    {
       ...
       $allowed_status = array('en_attente','valide','refuse','rejete','paye','a_regler','validee','refusee');

if (!isset($data['statut']) || !in_array($data['statut'], $allowed_status)) {
    $data['statut'] = 'en_attente'; 
}
        ...
        //try {
        if ($id) {
            //var_dump($t, $data, array( $pk=>$id ));exit();
            $result = $wpdb->update($t, $data, array( $pk => $id ));
    ...
    private static function process_csv_import($file_path, $entity_type, $mapping, $separator)
    {
        ...
        if ($entity_type === 'licences' && isset($_POST['club_id'])) {
            ...
            if ($club_id > 0) {
                ...
                if (!$club_exists) {
                    return array('success' => false, 'count' => 0, 'skipped' => 0, 'error' => 'Club invalide');
                }
            }
        }
        ...
        $file = fopen($file_path, 'r');
        if (!$file) {
            return array('success' => false, 'count' => 0, 'skipped' => 0);
        }
        ...
        return array('success' => true, 'count' => $count, 'skipped' => $skipped_empty);
    }
    /**
    ...
    */
    ...
    private static function is_date_column($table_name, $column_name)
    {
        ...
        if (!isset($column_types[$table_name])) {
            ...
            $column_types[$table_name] = array();

            foreach ($columns as $column) {
                $column_types[$table_name][$column['Field']] = array(
    ...
    */
    private static function convert_yes_no_to_boolean($value)
    {
        ...
        $true_values = array('oui', 'yes', 'true', 'vrai', '1', 'ok', 'activé', 'active', 'checked');
        $false_values = array('non', 'no', 'false', 'faux', '0', 'désactivé', 'inactive', 'unchecked');
