...
class UFSC_Stats {
    ...
    public static function get_club_stats( $club_id, $season = null ) {
        ...
        // Gender distribution
        $gender_rows = $wpdb->get_results( "SELECT sexe, COUNT(*) AS count FROM `{$table}` WHERE {$where} GROUP BY sexe" );
        $gender_counts = array();
        foreach ( $gender_rows as $row ) {
            $gender_counts[ $row->sexe ] = (int) $row->count;
        }
        

        // Practice distribution
        $practice_rows = $wpdb->get_results( "SELECT competition, COUNT(*) AS count FROM `{$table}` WHERE {$where} GROUP BY competition" );
        $practice_counts = array();
        foreach ( $practice_rows as $row ) {
            $practice_counts[ $row->competition ] = (int) $row->count;
        }
        // Birth year distribution
        $birth_rows = $wpdb->get_results( "SELECT YEAR(date_naissance) AS year, COUNT(*) AS count FROM `{$table}` WHERE {$where} GROUP BY YEAR(date_naissance)" );
        $birth_year_counts = array();
        foreach ( $birth_rows as $row ) {
            $birth_year_counts[ $row->year ] = (int) $row->count;
        }
        $birth_year_counts = array_filter($birth_year_counts, function($year) {
            return !empty($year) && $year != 0 && $year != "";
        ...
        return array(
            ...
            'by_birth_year'      => array_filter($birth_year_counts),
        );
    }
}
