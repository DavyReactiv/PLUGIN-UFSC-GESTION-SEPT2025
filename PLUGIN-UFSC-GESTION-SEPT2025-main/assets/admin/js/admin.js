...
jQuery(document).ready(function($) {
    ...
    //confirmation dialog only for the delete items
    jQuery(document).ready(function($) {
        $('.ufsc-gestion_page_ufsc-licences #bulk-actions-form').on('submit', function(e) {
            var selectedAction = $('#bulk-action-selector').val();
            var selectedCount = $('input[name="licence_ids[]"]:checked').length;
            ...
            if (selectedAction === 'delete') {
                var confirmation = confirm(
                    'Êtes-vous sûr de vouloir supprimer ' + selectedCount +
                    ' élément(s) ? Cette action est irréversible.'
                );
