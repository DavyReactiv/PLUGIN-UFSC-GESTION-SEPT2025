...
class Licence_Product {
    private $status; private $price; private $purchasable; private $type;
    public function __construct( $status, $price, $purchasable, $type = 'simple' ) { $this->status=$status; $this->price=$price; $this->purchasable=$purchasable; $this->type=$type; }
    ...
    public function get_type(){ return $this->type; } public function get_permalink(){ return 'https://example.test/licence'; }
}
...
$GLOBALS['opts']['ufsc_woocommerce_settings'] = array( 'product_license_id' => 2934 );
$settings = ufsc_get_woocommerce_settings();
$assert( 2934 === $settings['product_license_id'], 'canonical array is the sole licence product source' );
...
$GLOBALS['products'][2934] = new Licence_Product('publish','25',false);
$assert( 'Le produit #2934 est configuré mais n’est pas achetable.' === ufsc_get_licence_product_message(), 'not-purchasable diagnostic' );
$GLOBALS['products'][2934] = new Licence_Product('publish','25',true,'grouped');
$assert( 'product_type_not_allowed' === ufsc_get_licence_product_resolution()['unavailable_reason'], 'only simple products and variations are allowed' );
