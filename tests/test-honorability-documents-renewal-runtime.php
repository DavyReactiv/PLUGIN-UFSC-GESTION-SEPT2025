...
class FakeWpdb { public $rows=array(); function prepare($q,...$a){return false!==strpos($q,'GET_LOCK')||false!==strpos($q,'RELEASE_LOCK')?$q:end($a);} function get_row($id){return $this->rows[(int)$id]??null;} function get_results($q=''){return array_values($this->rows);} function get_col($q=''){return false!==strpos($q,'DESC')?array('id','club_id','season','role','is_included'):array('president','secretaire','tresorier','adherent','adherent','adherent','adherent','adherent','adherent','adherent');} function get_var($q){return 1;} function query($q){return 0;} }
