...
$roles = array( 'Président', 'Secrétaire', 'Trésorier', 'Dirigeant', 'Éducateur', 'Entraîneur', 'Coach', 'Encadrant', 'Responsable technique', 'Arbitre' );
foreach ( $roles as $role ) {
	if ( ! ufsc_role_requires_honorability( $role ) ) { fwrite( STDERR, "FAIL: {$role} should require honorability\n" ); exit( 1 ); }
}
if ( ufsc_role_requires_honorability( 'Pratiquant' ) ) { fwrite( STDERR, "FAIL: Pratiquant must be exempt\n" ); exit( 1 ); }
