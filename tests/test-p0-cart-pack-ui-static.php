...
foreach(array('ufsc_allocate_pack_credit','Cette licence est comprise dans votre pack','ajoutée au panier au tarif en vigueur','ufsc_get_licence_product_resolution') as $s)$assert(strpos($handler,$s)!==false,$s);
foreach(array('ufsc_ensure_woocommerce_cart','ufsc_persist_woocommerce_cart','ufsc_get_cart_product_arguments') as $s)$assert(strpos($cart,$s)!==false,"renewal $s");
