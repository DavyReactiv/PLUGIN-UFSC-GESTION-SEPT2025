...
class RuntimeCart{public $items=array();function get_cart(){return $this->items;}function add_to_cart($p,$q,$v,$a,$d){$this->items[]=array('product_id'=>$p,'quantity'=>$q)+$d;return 'line';}}
class RuntimeWoo{public $session=null,$cart=null,$session_calls=0,$cart_calls=0;function initialize_session(){$this->session=(object)array();$this->session_calls++;}function initialize_cart(){$this->cart=new RuntimeCart();$this->cart_calls++;}}
