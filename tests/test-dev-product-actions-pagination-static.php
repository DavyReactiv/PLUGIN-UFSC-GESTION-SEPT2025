...
$assert( false === strpos( $settings, "apply_filters( 'ufsc_licence_product_id'") && false !== strpos( $settings, 'ufsc_get_licence_product_resolution') && false === strpos( $settings, 'wc_get_products( array( \'limit\' => 1'), 'canonical setting is server-only and never guesses or filters a product' );
$assert( false !== strpos( $front, 'ufsc_get_licence_product_message( $product_resolution )') && false !== strpos( $front, 'data-ufsc-product-ready=') && false !== strpos( $front, 'disabled( ! $product_ready )'), 'cart action always rendered with unavailable explanation' );
...
$assert( false !== strpos( $css, '.ufsc-club-portal .ufsc-btn') && false !== strpos( $css, 'min-height: 44px') && false !== strpos( $css, ':focus-visible') && false === strpos( substr( $css, strpos( $css, '/* Portal Club UI:') ), '!important'), 'scoped visible accessible actions without blanket important overrides' );
