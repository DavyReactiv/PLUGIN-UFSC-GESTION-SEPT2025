 ...
 *
 * The option/filter remains authoritative so the federation can publish a new
 * version without another plugin deployment. The public 2026 form is only the
 * safe default when no URL has been configured in WordPress.
 */
 ...
 *
 * This is deliberately a presentation compatibility layer: no input name,
 * nonce, handler, licence rule, club rule or persistence model is replaced.
 * The existing `honorability_confirmed` checkbox is retained for server-side
 * compatibility but its visible wording is clarified so it never pretends that
...
function ufsc_render_honorability_onboarding_assets() {
    ...
    $template_url = function_exists( 'ufsc_get_honorability_template_url' )
        ? ufsc_get_honorability_template_url()
        : ufsc_honorability_default_template_url( '' );
    $note_url = ufsc_get_honorability_note_url();
    ?>
    <style id="ufsc-honorability-onboarding-css">
    .ufsc-honorability-onboarding{margin:14px 0;padding:14px 16px;border:1px solid #dfe5ef;border-left:4px solid #b48755;border-radius:10px;background:#fff;color:#2f3440}
    .ufsc-honorability-onboarding__title{margin:0 0 5px;font-size:15px;font-weight:800;color:#292668}
    .ufsc-honorability-onboarding__text{margin:0 0 10px;font-size:13px;line-height:1.45}
    .ufsc-honorability-onboarding__actions{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 9px}
    .ufsc-honorability-onboarding__btn{display:inline-flex;align-items:center;justify-content:center;min-height:36px;padding:7px 12px;border:1px solid #0b6194;border-radius:999px;background:#0b6194;color:#fff!important;text-decoration:none!important;font-size:12px;font-weight:800}
    .ufsc-honorability-onboarding__btn--secondary{background:#fff;color:#0b6194!important}
    .ufsc-honorability-onboarding__steps{margin:0;padding-left:19px;font-size:12px;line-height:1.5;color:#525967}
    .ufsc-honorability-onboarding__later{margin:8px 0 0;padding:8px 10px;border-radius:7px;background:#fff6dc;color:#6c5013;font-size:12px;font-weight:650}
    @media(max-width:640px){.ufsc-honorability-onboarding__actions{display:grid}.ufsc-honorability-onboarding__btn{width:100%}}
    </style>
    <script id="ufsc-honorability-onboarding-js">
    (function(){
        'use strict';
        var templateUrl = <?php echo wp_json_encode( esc_url_raw( $template_url ) ); ?>;
        ...
        function hasWords(node, words){
            ...
            return words.some(function(word){ return text.indexOf(norm(word)) !== -1; });
        }
        function makeBox(context){
            var box = document.createElement('div');
            box.className = 'ufsc-honorability-onboarding';
            box.setAttribute('data-ufsc-honorability-onboarding', context);
            var later = context === 'club'
                ? 'Le dépôt ne bloque pas la création du club. Après enregistrement des dirigeants, chaque personne concernée retrouvera son attestation à déposer dans le Compte Club.'
                : 'Si cette personne n’est pas encore enregistrée, enregistrez d’abord la licence. Le dépôt du document signé sera ensuite disponible sur son profil dans le Compte Club.';
            box.innerHTML = ''+
                '<p class="ufsc-honorability-onboarding__title">Honorabilité : document à préparer</p>'+
                '<p class="ufsc-honorability-onboarding__text">Les dirigeants, éducateurs, entraîneurs, coaches et encadrants concernés doivent disposer d’une attestation d’honorabilité pour la saison.</p>'+
                '<div class="ufsc-honorability-onboarding__actions">'+
                    '<a class="ufsc-honorability-onboarding__btn" href="'+templateUrl+'" target="_blank" rel="noopener">Télécharger l’attestation UFSC remplissable</a>'+
                    '<a class="ufsc-honorability-onboarding__btn ufsc-honorability-onboarding__btn--secondary" href="'+noteUrl+'" target="_blank" rel="noopener">Lire la note sur le contrôle de l’honorabilité</a>'+
                '</div>'+
                '<ol class="ufsc-honorability-onboarding__steps"><li>Télécharger et remplir l’attestation.</li><li>La signer sur ordinateur ou à la main.</li><li>Enregistrer le document signé en PDF, JPG ou PNG.</li><li>Le déposer sur le profil de la personne concernée.</li></ol>'+
