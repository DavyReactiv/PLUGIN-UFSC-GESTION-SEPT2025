...
if ( ! function_exists( 'ufsc_get_club_profile_value' ) ) {
    /** Read club profile values with historical column aliases, without writing to DB. */
    function ufsc_get_club_profile_value( $club, $field ) {
        $aliases = array(
            'name'        => array( 'nom', 'name', 'club_name' ),
            'region'      => array( 'region', 'region_club' ),
            'address'     => array( 'adresse', 'adresse_siege', 'address' ),
            ...
            'affiliation_number' => array( 'num_affiliation', 'numero_affiliation' ),
