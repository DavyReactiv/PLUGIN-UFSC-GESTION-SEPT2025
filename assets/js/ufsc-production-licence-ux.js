(function () {
  ...
  function insertQueryNotice() { var params=new URLSearchParams(window.location.search||''),error=params.get('ufsc_error'),message=params.get('ufsc_message'); if(document.querySelector('.ufsc-notice-error,.ufsc-alert.error,.ufsc-message.ufsc-error'))return; if(error)showNotice(error,'error');else if(message)showNotice(mapMessage(message)||message.replace(/[_-]+/g,' '),message.indexOf('pending')!==-1||message.indexOf('waiting')!==-1?'pending':'success'); }
