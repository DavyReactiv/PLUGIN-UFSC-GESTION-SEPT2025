...
class UFSC_Mail_Service {
    ...
    public static function register_menu() {
        add_submenu_page(
            'ufsc-dashboard',
            __( 'Communication clubs', 'ufsc-clubs' ),
            __( 'Communication clubs', 'ufsc-clubs' ),
            self::capability(),
    ...
    public static function render_admin() {
        ...
        global $wpdb;
        $campaigns_table = $wpdb->prefix . 'ufsc_mail_campaigns';
        $campaigns       = $wpdb->get_results( "SELECT * FROM {$campaigns_table} ORDER BY id DESC LIMIT 30" );
    ...
    public static function handle_retry_failed() {
        ...
        if ( $campaign_id > 0 ) {
            ...
            if ( class_exists( 'UFSC_Audit_Logger' ) ) {
                UFSC_Audit_Logger::log( 'UFSC Mail retry failed for campaign #' . $campaign_id );
            }
        }
    ...
    private static function create_campaign_from_form( $data, $preview ) {
        ...
        $hash = md5( strtolower( $data['subject'] . '|' . $data['target_type'] . '|' . wp_strip_all_tags( $data['message_html'] ) ) );
    ...
    private static function get_targeted_recipients( $target ) {
        ...
        foreach ( (array) $rows as $row ) {
            ...
            if ( in_array( $status, array( 'supprime', 'supprimé', 'corbeille', 'deleted', 'trash' ), true ) ) { $ignored[] = array( 'club' => (string) $row->nom, 'reason' => 'club supprimé/corbeille' ); continue; }
            $email_raw = (string) ( $row->email ?? '' );
            if ( '' === trim( $email_raw ) ) { $ignored[] = array( 'club' => (string) $row->nom, 'reason' => 'email absent' ); continue; }
            $email = sanitize_email( $email_raw );
            if ( ! is_email( $email ) ) { $ignored[] = array( 'club' => (string) $row->nom, 'reason' => 'email invalide' ); continue; }
            if ( isset( $seen[ strtolower( $email ) ] ) ) { $ignored[] = array( 'club' => (string) $row->nom, 'reason' => 'doublon email' ); continue; }
            if ( 'affiliated_up_to_date' === $target && function_exists( 'ufsc_is_affiliation_renewed' ) && ! ufsc_is_affiliation_renewed( (int) $row->id, $current ) ) {
                $ignored[] = array( 'club' => (string) $row->nom, 'reason' => 'affiliation non à jour' );
                continue;
            }
    ...
    public static function process_queue( $limit = 15, $campaign_id = 0 ) {
        ...
        $params[] = (int) $limit;
        $sql = "SELECT * FROM {$queue} WHERE {$where} ORDER BY id ASC LIMIT %d";
        ...
        foreach ( (array) $rows as $row ) {
            ...
            $message = self::render_message( $campaign->message_html, $vars );
            $ok = self::send_html_mail( $row->recipient_email, $campaign->subject, $message );
            if ( $ok ) {
    ...
    private static function refresh_campaign_statuses() {
        ...
        foreach ( (array) $all as $c ) {
            ...
            $wpdb->update( $campaigns, array( 'sent_count' => $sent, 'failed_count' => $failed, 'total_recipients' => $total, 'status' => $status, 'updated_at' => current_time( 'mysql' ), 'completed_at' => ( 0 === $pending ? current_time( 'mysql' ) : null ) ), array( 'id' => $id ) );
        }
    }
    ...
    private static function send_html_mail( $to, $subject, $message ) {
        $from_name = apply_filters( 'ufsc_mail_from_name', 'UFSC France' );
        $from_email = apply_filters( 'ufsc_mail_from_email', 'infos@ufsc-france.org' );
        $reply_to = apply_filters( 'ufsc_mail_reply_to', 'secretariat@ufsc-france.org' );
