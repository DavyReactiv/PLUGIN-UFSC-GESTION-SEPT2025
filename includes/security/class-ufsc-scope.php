...
class UFSC_Scope {
    ...
    /**
     * Append scope condition to an array of WHERE conditions.
     *
     ...
     * @return array
     */
    public static function apply_scope_to_where( array $conditions, $column, $table_alias = '' ) {
        $scope_condition = self::build_scope_condition( $column, $table_alias );
        if ( $scope_condition ) {
    ...
    /**
     * Apply scope to argument arrays (e.g. filters).
     *
     * @param array  $args
     * @param string $key
     * @return array
     */
    public static function apply_scope_to_args( array $args, $key = 'region' ) {
        $slug = self::get_user_scope_region();
        if ( ! $slug ) {
            return $args;
        }
    ...
    /**
     * Get licence region based on club linkage or licence region.
     *
     * @param int $licence_id
     * @return string|null
     */
    public static function get_licence_region( $licence_id ) {
        global $wpdb;
        $settings = UFSC_SQL::get_settings();
        ...
        $club_id = (int) $wpdb->get_var( $wpdb->prepare(
            "SELECT club_id FROM `{$licences_table}` WHERE `{$pk}` = %d",
            $licence_id
        ) );
        if ( $club_id ) {
            return self::get_club_region( $club_id );
        }

        $region = $wpdb->get_var( $wpdb->prepare(
            "SELECT region FROM `{$licences_table}` WHERE `{$pk}` = %d",
            $licence_id
        ) );
    ...
    /**
     * Enforce scope for club/licence/entry IDs or region value.
     *
     * @param array|int $args
     * @return void
     */
    public static function enforce_object_scope( $args ) {
        if ( is_int( $args ) ) {
            self::assert_club_in_scope( $args );
            return;
        }
        ...
        if ( isset( $args['club_id'] ) ) {
            self::assert_club_in_scope( (int) $args['club_id'] );
            return;
        }

        if ( isset( $args['licence_id'] ) ) {
            $region = self::get_licence_region( (int) $args['licence_id'] );
            self::assert_in_scope( $region );
            return;
        }

        if ( isset( $args['entry_id'] ) ) {
            if ( function_exists( 'ufsc_get_entry_region' ) ) {
                $region = ufsc_get_entry_region( (int) $args['entry_id'] );
                self::assert_in_scope( $region );
            }
            return;
        }
