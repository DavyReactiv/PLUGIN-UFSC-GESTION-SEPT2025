...
/**
 * Matrices FFST multi-clubs (lecture seule).
 */
final class UFSC_FFST_Matrix_Export_Admin {
    ...
    public static function render_panel() {
        ...
        <style id="ufsc-ffst-matrix-export-style">
            ...
            .ufsc-ffst-matrix h2{margin:0 0 6px}.ufsc-ffst-matrix p{margin:0 0 14px;color:#50575e}.ufsc-ffst-matrix label{font-weight:600}
            .ufsc-ffst-matrix__toolbar{display:grid;grid-template-columns:180px minmax(220px,1fr) auto;gap:10px;align-items:end;margin-bottom:12px}.ufsc-ffst-matrix__toolbar input{width:100%;min-height:36px}
            ...
            .ufsc-ffst-matrix__club{display:grid;grid-template-columns:30px minmax(180px,1fr) 170px 150px;gap:10px;align-items:center;padding:9px 12px;border-bottom:1px solid #e7e7e7;background:#fff}.ufsc-ffst-matrix__club:last-child{border-bottom:0}.ufsc-ffst-matrix__club-name{font-weight:600}.ufsc-ffst-matrix__muted{color:#646970;font-size:12px}.ufsc-ffst-matrix__actions{display:flex;flex-wrap:wrap;gap:8px}.ufsc-ffst-matrix__count{font-weight:700;color:#135e96}
        ...
        <div class="notice notice-info ufsc-ffst-matrix">
            ...
            <p><?php echo esc_html__( 'Sélectionnez un ou plusieurs clubs puis générez une matrice affiliations, dirigeants ou pratiquants. Ces exports n’écrivent aucune donnée et restent indépendants des documents officiels déjà en place.', 'ufsc-clubs' ); ?></p>
    ...
    public static function handle_export() {
        ...
        if ( 'affiliations' === $type ) { $ss = self::build_affiliations_workbook( $clubs, $season ); }
        elseif ( 'dirigeants' === $type ) { $ss = self::build_leaders_workbook( $clubs, $season ); }
        else { $ss = self::build_practitioners_workbook( $clubs, $season ); }

        self::send_workbook( $ss, 'matrice-ffst-' . $type . '-' . sanitize_title( $season ) . '.xlsx' );
    }
    ...
    private static function build_affiliations_workbook( array $clubs, $season ) {
        ...
        $sheet->setCellValue( 'A1', 'Matrice FFST ' . $season . ' — Affiliation / Réaffiliation' ); $sheet->mergeCells( 'A1:DA1' );
        $sheet->setCellValue( 'A2', 'Export généré depuis UFSC Gestion. Les champs non disponibles restent vides et sont signalés dans la feuille Contrôle.' ); $sheet->mergeCells( 'A2:DA2' );
        $sheet->fromArray( $headers, null, 'A4' );
        $row = 5; foreach ( $clubs as $club ) { $sheet->fromArray( self::affiliation_row( $club ), null, 'A' . $row++ ); }
        self::style_sheet( $sheet, count( $headers ), $row - 1, 4 );

        $dict = $ss->createSheet(); $dict->setTitle( 'Dictionnaire champs' );
        $dict->fromArray( array( 'Ordre', 'Section', 'Champ Excel', 'Remarque' ), null, 'A1' );
        $r = 2; $i = 1; foreach ( $headers as $header ) { $dict->fromArray( array( $i++, self::affiliation_section( $header ), $header, self::field_remark( $header ) ), null, 'A' . $r++ ); }
        self::style_sheet( $dict, 4, $r - 1, 1 );
        ...
        self::style_sheet( $reserved, 2, 10, 1 );

        self::add_control_sheet( $ss, $clubs, $season, 'Affiliation' ); $ss->setActiveSheetIndex( 0 ); return $ss;
    }

    private static function build_leaders_workbook( array $clubs, $season ) {
        $ss = new \PhpOffice\PhpSpreadsheet\Spreadsheet(); $sheet = $ss->getActiveSheet(); $sheet->setTitle( 'Dirigeants' );
        ...
        $sheet->fromArray( $headers, null, 'A1' ); $row = 2;
        foreach ( $clubs as $club ) { $licences = self::get_licences_for_club( absint( self::value( $club, array( 'id', 'club_id' ) ) ), $season ); foreach ( self::leader_rows( $club, $licences, $season ) as $data ) { $sheet->fromArray( $data, null, 'A' . $row++ ); } }
        self::style_sheet( $sheet, count( $headers ), $row - 1, 1 ); self::add_control_sheet( $ss, $clubs, $season, 'Dirigeants' ); $ss->setActiveSheetIndex( 0 ); return $ss;
    }

    private static function build_practitioners_workbook( array $clubs, $season ) {
        $ss = new \PhpOffice\PhpSpreadsheet\Spreadsheet(); $sheet = $ss->getActiveSheet(); $sheet->setTitle( 'Pratiquants' );
        ...
        $sheet->fromArray( $headers, null, 'A1' ); $row = 2;
        foreach ( $clubs as $club ) { $club_id = absint( self::value( $club, array( 'id', 'club_id' ) ) ); foreach ( self::get_licences_for_club( $club_id, $season ) as $licence ) { if ( self::is_leader_licence( $licence ) ) { continue; } $sheet->fromArray( self::practitioner_row( $club, $licence, $season ), null, 'A' . $row++ ); } }
        self::style_sheet( $sheet, count( $headers ), $row - 1, 1 ); self::add_control_sheet( $ss, $clubs, $season, 'Pratiquants' ); $ss->setActiveSheetIndex( 0 ); return $ss;
    }
    ...
    private static function affiliation_row( $club ) {
        $aff = self::value( $club, array( 'numero_affiliation_ffst' ) ); $disciplines = self::split_values( self::value( $club, array( 'disciplines_ffst', 'disciplines', 'discipline' ) ) ); $codes = self::split_values( self::value( $club, array( 'codes_disciplines_ffst', 'code_discipline' ) ) );
        ...
        foreach ( array( 'president', 'secretaire', 'tresorier', 'entraineur' ) as $prefix ) { $row[] = self::value( $club, array( $prefix . '_nom' ) ); $row[] = self::value( $club, array( $prefix . '_prenom' ) ); $row[] = self::value( $club, array( $prefix . '_date_naissance' ) ); $row[] = self::value( $club, array( $prefix . '_ville_naissance' ) ); $row[] = self::join_non_empty( array( self::value( $club, array( $prefix . '_departement_naissance' ) ), self::value( $club, array( $prefix . '_pays_naissance' ) ) ), ' / ' ); $row[] = self::parent_identity_value( $club, $prefix, 'pere' ); $row[] = self::parent_identity_value( $club, $prefix, 'mere' ); $row[] = self::value( $club, array( $prefix . '_adresse' ) ); $row[] = self::value( $club, array( $prefix . '_complement_adresse' ) ); $row[] = self::value( $club, array( $prefix . '_code_postal' ) ); $row[] = self::value( $club, array( $prefix . '_ville' ) ); $row[] = self::value( $club, array( $prefix . '_tel', $prefix . '_telephone' ) ); $row[] = self::value( $club, array( $prefix . '_email' ) ); }
        for ( $i = 0; $i < 13; $i++ ) { $row[] = ''; }
        ...
        for ( $i = 0; $i < 5; $i++ ) { $row[] = ''; }
        $row[] = self::join_non_empty( array( self::value( $club, array( 'signataire_prenom' ) ), self::value( $club, array( 'signataire_nom' ) ), self::value( $club, array( 'signataire_qualite' ) ) ), ' - ' );
        return array_slice( array_pad( $row, 105, '' ), 0, 105 );
    }

    private static function leader_rows( $club, array $licences, $season ) {
        $rows = array(); foreach ( array( 'president' => 'Président', 'secretaire' => 'Secrétaire', 'tresorier' => 'Trésorier', 'entraineur' => 'Entraîneur / instructeur' ) as $prefix => $label ) {
            $nom=self::value($club,array($prefix.'_nom'));$prenom=self::value($club,array($prefix.'_prenom')); if(''===trim($nom.$prenom)){continue;} $lic=self::find_person_licence($licences,$nom,$prenom,$label); $date=self::value($club,array($prefix.'_date_naissance')); $missing=array(); if(''===trim($nom))$missing[]='nom'; if(''===trim($prenom))$missing[]='prénom'; if(''===trim($date))$missing[]='date de naissance';
            $rows[]=array(self::value($club,array('nom','name','club_name')),self::value($club,array('numero_affiliation_ffst')),$season,$label,$nom,$prenom,$date,self::value($club,array($prefix.'_ville_naissance')),self::value($club,array($prefix.'_departement_naissance')),self::value($club,array($prefix.'_pays_naissance')),self::value($club,array($prefix.'_pere_nom_prenom')),self::value($club,array($prefix.'_mere_nom_prenom')),self::value($club,array($prefix.'_adresse')),self::value($club,array($prefix.'_complement_adresse')),self::value($club,array($prefix.'_code_postal')),self::value($club,array($prefix.'_ville')),self::value($club,array($prefix.'_tel',$prefix.'_telephone')),self::value($club,array($prefix.'_email')),$lic?self::value($lic,array('numero_licence_ufsc','numero_licence','licence_number')):'',$lic?self::value($lic,array('numero_licence_ffst','licence_ffst')):'',$missing?'À compléter : '.implode(', ',$missing):'OK');
        } return $rows;
    }

    private static function practitioner_row( $club, $licence, $season ) {
        $nom=self::value($licence,array('nom','nom_licence','last_name'));$prenom=self::value($licence,array('prenom','first_name'));$date=self::value($licence,array('date_naissance','birth_date','dob'));$adresse=self::value($licence,array('adresse','address'));$missing=array();if(''===trim($nom))$missing[]='nom';if(''===trim($prenom))$missing[]='prénom';if(''===trim($date))$missing[]='date de naissance';if(''===trim($adresse))$missing[]='adresse';
        return array(self::value($club,array('nom','name','club_name')),self::value($club,array('numero_affiliation_ffst')),$season,self::value($licence,array('discipline','discipline_nom','sport'))?:self::value($club,array('disciplines_ffst')),self::value($licence,array('code_discipline_ffst','code_discipline'))?:self::value($club,array('codes_disciplines_ffst')),self::value($licence,array('numero_licence_ufsc','numero_licence','licence_number')),self::value($licence,array('numero_licence_ffst','licence_ffst')),$nom,$prenom,self::value($licence,array('sexe','genre','gender')),$date,$adresse,self::value($licence,array('complement_adresse','address2')),self::value($licence,array('code_postal','postal_code','cp')),self::value($licence,array('ville','city')),self::value($licence,array('telephone','tel','phone')),self::value($licence,array('email','mail')),'P',self::value($licence,array('type_pratique','pratique','competition_loisir')),self::value($licence,array('statut','status')),$missing?'À compléter : '.implode(', ',$missing):'OK');
    }

    private static function is_leader_licence( $licence ) { $role=self::normalize(self::value($licence,array('role','fonction','poste','position'))); foreach(array('president','secretaire','tresorier','entraineur','instructeur','dirigeant') as $needle){if(false!==strpos($role,$needle))return true;} return false; }
    private static function find_person_licence( array $licences, $nom, $prenom, $role_label ) { $n=self::normalize($nom);$p=self::normalize($prenom);foreach($licences as $lic){if($n&&$p&&self::normalize(self::value($lic,array('nom','nom_licence','last_name')))===$n&&self::normalize(self::value($lic,array('prenom','first_name')))===$p)return $lic;} $r=self::normalize($role_label);foreach($licences as $lic){$lr=self::normalize(self::value($lic,array('role','fonction','poste','position')));if($lr&&(false!==strpos($r,$lr)||false!==strpos($lr,$r)))return $lic;} return null; }

    private static function get_clubs() { global $wpdb; $table=self::clubs_table(); if(!$table)return array(); return (array)$wpdb->get_results("SELECT * FROM `{$table}` ORDER BY nom ASC"); } // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery
    private static function get_clubs_by_ids( array $ids ) { global $wpdb; $table=self::clubs_table(); if(!$table)return array(); $ids=array_values(array_unique(array_filter(array_map('absint',$ids)))); if(!$ids)return array(); $placeholders=implode(',',array_fill(0,count($ids),'%d')); return (array)$wpdb->get_results($wpdb->prepare("SELECT * FROM `{$table}` WHERE id IN ({$placeholders}) ORDER BY nom ASC",...$ids)); } // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
    private static function get_licences_for_club( $club_id, $season ) { global $wpdb; $table=self::licences_table(); if(!$table)return array(); $columns=function_exists('ufsc_table_columns')?(array)ufsc_table_columns($table):array(); $club_col=in_array('club_id',$columns,true)?'club_id':(in_array('id_club',$columns,true)?'id_club':''); if(!$club_col)return array(); foreach(array('season','saison','paid_season','season_end_year') as $sc){if(!in_array($sc,$columns,true))continue;if('season_end_year'===$sc){$end=preg_match('/^\d{4}-(\d{4})$/',$season,$m)?(int)$m[1]:0;return $end?(array)$wpdb->get_results($wpdb->prepare("SELECT * FROM `{$table}` WHERE `{$club_col}`=%d AND `season_end_year`=%d",$club_id,$end)):array();}return (array)$wpdb->get_results($wpdb->prepare("SELECT * FROM `{$table}` WHERE `{$club_col}`=%d AND REPLACE(TRIM(`{$sc}`), '/', '-')=%s",$club_id,$season));} return array(); }
    ...
    private static function parent_identity_value( $row, $prefix, $parent ) { $nom=self::value($row,array($prefix.'_'.$parent.'_nom'));$prenom=self::value($row,array($prefix.'_'.$parent.'_prenom'));$split=self::join_non_empty(array($nom,$prenom),' ');return ''!==$split?$split:self::value($row,array($prefix.'_'.$parent.'_nom_prenom')); }

    private static function add_control_sheet( $ss, array $clubs, $season, $type ) { $sheet=$ss->createSheet();$sheet->setTitle('Contrôle');$sheet->fromArray(array('Club','Saison','Type export','N° affiliation FFST','Observation'),null,'A1');$row=2;foreach($clubs as $club){$missing=array();foreach(array('nom'=>'nom club','adresse'=>'adresse','code_postal'=>'code postal','ville'=>'ville') as $field=>$label){if(''===trim(self::value($club,array($field))))$missing[]=$label;}$sheet->fromArray(array(self::value($club,array('nom','name','club_name')),$season,$type,self::value($club,array('numero_affiliation_ffst')),$missing?'À compléter : '.implode(', ',$missing):'OK'),null,'A'.$row++);}self::style_sheet($sheet,5,$row-1,1); }
    private static function style_sheet( $sheet, $column_count, $last_row, $header_row ) { if($column_count<1)return;$last_col=\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($column_count);$sheet->freezePane('A'.($header_row+1));$sheet->getStyle('A'.$header_row.':'.$last_col.$header_row)->getFont()->setBold(true)->getColor()->setARGB('FFFFFFFF');$sheet->getStyle('A'.$header_row.':'.$last_col.$header_row)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FF135E96');$sheet->getStyle('A'.$header_row.':'.$last_col.max($header_row,$last_row))->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_TOP)->setWrapText(true);for($c=1;$c<=$column_count;$c++){$sheet->getColumnDimension(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($c))->setWidth($column_count>40?22:20);}$sheet->setAutoFilter('A'.$header_row.':'.$last_col.$header_row); }
    private static function send_workbook( $ss, $filename ) { $tmp=wp_tempnam($filename);if(!$tmp){wp_die(esc_html__('Impossible de créer le fichier temporaire.','ufsc-clubs'),'',array('response'=>500));}$writer=new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($ss);$writer->save($tmp);nocache_headers();header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');header('Content-Disposition: attachment; filename="'.sanitize_file_name($filename).'"');header('Content-Length: '.filesize($tmp));readfile($tmp);@unlink($tmp);exit; } // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_readfile
