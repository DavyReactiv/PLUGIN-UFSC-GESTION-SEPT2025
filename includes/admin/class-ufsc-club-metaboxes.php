...
class UFSC_CL_Club_Metaboxes {
    ...
    public static function render_renewal_metabox( $post ) {
        ...
        $renewed = function_exists( 'ufsc_is_club_affiliated_for_season' ) ? ufsc_is_club_affiliated_for_season( $club_id, $current_season ) : false;
        ...
        if ( ! $renewed && $aff_season !== $current_season && ! empty( $wc_settings['product_affiliation_id'] ) ) {
            ...
            echo '<input type="hidden" name="ufsc_target_season" value="' . esc_attr( $current_season ) . '">';
            ...
            echo '<p>' . esc_html( sprintf( __( 'Renouvellement %1$s ouvert à partir du %2$s', 'ufsc-clubs' ), $current_season, $renew_open_label ) ) . '</p>';
        }
    }
