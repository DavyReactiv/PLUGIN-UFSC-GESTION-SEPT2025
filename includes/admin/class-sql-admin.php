...
class UFSC_SQL_Admin
{
     ...
     * @return string
     */
    private static function build_licences_redirect_url( $extra_args = array() ) {
        $base_url = self::get_licences_admin_page_url();
        $referer  = wp_get_referer();

        if ( $referer ) {
            $referer_query = wp_parse_url( (string) $referer, PHP_URL_QUERY );
            $referer_args  = array();
            if ( $referer_query ) {
                wp_parse_str( $referer_query, $referer_args );
            }
            $referer_page = isset( $referer_args['page'] ) ? sanitize_key( (string) $referer_args['page'] ) : '';
            if ( in_array( $referer_page, self::get_licences_admin_page_slugs(), true ) ) {
                $base_url = $referer;
            }
        }

        return add_query_arg( $extra_args, remove_query_arg( array( 'updated', 'deleted', 'deleted_id', 'error' ), $base_url ) );
    }
     ...
     * @return string
     */
    private static function add_admin_licence_action_url_args( $url ) {
        $args = array( '_ufsc_ts' => time() );
    ...
    /**
     * Current admin URL for diagnostics only.
     *
     * @return string
     */
    private static function get_current_admin_url_for_debug() {
        ...
        return $host ? esc_url_raw( $scheme . $host . $request_uri ) : esc_url_raw( $request_uri );
    }

    /**
     * Render one row of the temporary licence ID diagnostic in the actions column.
     *
     * @param object $row        Current list row.
     * @param int    $licence_id Resolved row ID.
     * @param string $pk         Configured primary key.
     * @param string $admin_pk   Admin primary key used by view/edit.
     * @param string $view_url   Generated view URL.
     * @param string $edit_url   Generated edit URL.
     * @return void
     */
    private static function render_licence_row_id_debug( $row, $licence_id, $pk, $admin_pk, $view_url, $edit_url ) {
        if ( ! self::is_licence_id_debug_enabled() ) {
            return;
        }

        $row_id = isset( $row->id ) ? absint( $row->id ) : 0;
        echo '<div class="ufsc-id-debug" style="margin-top:6px;padding:6px;border-left:3px solid #d63638;background:#fff8e5;font-size:11px;line-height:1.45;word-break:break-all;">';
        echo '<strong>' . esc_html__( 'Debug IDs', 'ufsc-clubs' ) . '</strong><br />';
        echo 'row_id=' . esc_html( (string) $row_id ) . ' / licence_id=' . esc_html( (string) absint( $licence_id ) ) . '<br />';
        echo 'pk=' . esc_html( (string) $pk ) . ' / admin_pk=' . esc_html( (string) $admin_pk ) . '<br />';
        echo 'view_id=' . esc_html( (string) absint( $licence_id ) ) . ' / edit_id=' . esc_html( (string) absint( $licence_id ) ) . '<br />';
        echo 'view_url=' . esc_html( $view_url ) . '<br />';
        echo 'edit_url=' . esc_html( $edit_url );
        echo '</div>';
    }

    /**
     * Render the temporary licence fiche ID diagnostic.
     *
     * @param int         $requested_id Requested URL ID.
     * @param object|null $row          Loaded licence row.
     * @param string      $admin_pk     Primary key used to load the row.
     * @param string      $source       Whether the row was preloaded or reloaded.
     * @return void
     */
    private static function render_licence_form_id_debug( $requested_id, $row, $admin_pk, $source ) {
        if ( ! self::is_licence_id_debug_enabled() ) {
            return;
        }

        $loaded_id = ( is_object( $row ) && '' !== $admin_pk && isset( $row->{$admin_pk} ) ) ? absint( $row->{$admin_pk} ) : 0;
        $numero = '';
        foreach ( array( 'numero_licence', 'num_licence', 'licence_number', 'numero' ) as $field ) {
            if ( is_object( $row ) && isset( $row->{$field} ) && '' !== trim( (string) $row->{$field} ) ) {
                $numero = (string) $row->{$field};
                break;
            }
        }

        echo '<div class="notice notice-warning inline ufsc-id-debug"><p><strong>' . esc_html__( 'Diagnostic temporaire IDs licence', 'ufsc-clubs' ) . '</strong><br />';
        echo 'GET id reçu=' . esc_html( (string) absint( $requested_id ) ) . '<br />';
        echo 'primary key utilisée=' . esc_html( (string) $admin_pk ) . '<br />';
        echo 'ID chargé en base=' . esc_html( (string) $loaded_id ) . '<br />';
        echo 'nom chargé=' . esc_html( is_object( $row ) ? (string) ( $row->nom ?? '' ) : '' ) . '<br />';
        echo 'prénom chargé=' . esc_html( is_object( $row ) ? (string) ( $row->prenom ?? '' ) : '' ) . '<br />';
        echo 'numero_licence=' . esc_html( $numero ) . '<br />';
        echo 'return_to=' . esc_html( isset( $_GET['return_to'] ) ? 'présent' : 'absent' ) . '<br />';
        echo 'source=' . esc_html( $source ) . '<br />';
        echo 'url=' . esc_html( self::get_current_admin_url_for_debug() );
        echo '</p></div>';
    }
    ...
    private static function render_club_form($id, $readonly = false)
    {
        ...
        $sections = array(
            ...
            array(
                ...
                '',
                array(
                    // Président — identité, naissance et adresse détaillée.
    ...
    /* ---------------- Licences ---------------- */
    private static function debug_log_licences( $context, $payload = array() ) {
    ...
    private static function build_licence_visibility_condition( $alias, $licence_columns, $visibility = 'active' ) {
        ...
        if ( 'trash' === $visibility ) {
            return "({$field} IS NOT NULL AND {$field} != '0000-00-00 00:00:00')";
        }
        if ( 'all' === $visibility ) {
            return '';
        }
        return "({$field} IS NULL OR {$field} = '0000-00-00 00:00:00')";
    }

    private static function get_licence_filters_from_request( $source = 'get' ) {
    ...
    private static function build_licence_where_conditions( $filters, $licence_columns, $club_columns, $has_club_id ) {
        ...
        if (! empty($search)) {
            ...
            if (! empty($search_parts)) {
                $where_conditions[] = $wpdb->prepare( '(' . implode(' OR ', $search_parts) . ')', ...$search_values );
            }
        }
        ...
        if ( ! empty( $filter_region ) ) {
            if ( $has_club_id && in_array( 'region', $club_columns, true ) ) {
                if ( in_array( 'region', $licence_columns, true ) ) {
                    ...
                    $where_conditions[] = $wpdb->prepare( "NULLIF(c.region,'') = %s", $filter_region );
                }
        ...
        if ( 'all' !== $filter_season ) {
            $season_column = self::get_first_existing_column( $licence_columns, array( 'season', 'saison', 'paid_season', 'season_end_year' ) );
    ...
    /**
     * Return the first existing column from a preferred list as a safe SQL alias.
     *
     * @param string[] $columns Existing table columns.
     * @param string[] $candidates Candidate column names.
     * @param string   $alias SQL alias to expose in the result row.
     * @return string
     */
    private static function build_first_existing_select_column( array $columns, array $candidates, $alias ) {
        foreach ( $candidates as $column ) {
            if ( in_array( $column, $columns, true ) ) {
                return "l.{$column} AS {$alias}";
            }
        }
     ...
     * @return string
     */
    private static function get_licence_row_season_label( $licence ) {
        if ( function_exists( 'ufsc_get_licence_season_label' ) ) {
            $season = ufsc_get_licence_season_label( $licence );
            if ( is_string( $season ) && '' !== trim( $season ) ) {
                return $season;
            }
        }

        foreach ( array( 'season_resolved', 'saison', 'season', 'paid_season', 'season_end_year' ) as $property ) {
            if ( isset( $licence->{$property} ) && '' !== trim( (string) $licence->{$property} ) ) {
                return (string) $licence->{$property};
            }
        }
	...
	: (
		in_array( 'region', $licence_columns, true )
            ...
            self::build_first_existing_select_column( $licence_columns, array( 'numero_licence', 'num_licence', 'licence_number', 'numero' ), 'numero_licence_resolved' ),
            self::build_first_existing_select_column( $licence_columns, array( 'numero_asptt', 'num_asptt', 'asptt_number', 'numero_licence_delegataire' ), 'numero_asptt_resolved' ),
            self::build_first_existing_select_column( $licence_columns, array( 'categorie', 'category', 'type_licence', 'cat' ), 'categorie_resolved' ),
            self::build_first_existing_select_column( $licence_columns, array( 'season', 'saison', 'paid_season', 'season_end_year' ), 'season_resolved' ),
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			$debug_rows = array();
			foreach ( (array) $rows as $debug_row ) {
				$debug_context = function_exists( 'ufsc_get_licence_season_context_status' ) ? ufsc_get_licence_season_context_status( $debug_row, self::get_admin_current_season_label() ) : array();
				$debug_rows[] = array( 'id' => self::get_admin_licence_row_id( $debug_row, $admin_pk ), 'season' => self::get_licence_row_season_label( $debug_row ), 'context' => $debug_context['renewal_state'] ?? 'unknown' );
			}
			error_log( '[UFSC licence season] ' . wp_json_encode( array( 'requested' => $filter_season, 'normalized' => '__current' === $filter_season ? self::get_admin_current_season_label() : $filter_season, 'column' => self::get_first_existing_column( $licence_columns, array( 'season', 'saison', 'paid_season', 'season_end_year' ) ), 'condition' => self::build_licence_season_condition( self::get_first_existing_column( $licence_columns, array( 'season', 'saison', 'paid_season', 'season_end_year' ) ), $filter_season ), 'current' => self::get_admin_current_season_label(), 'rows' => $debug_rows ) ) );
		}
        self::debug_log_licences(
            'admin_list',
            array(
                'filters' => $filters,
                'total_items' => (int) $total_items,
                'rows_current_page' => is_array( $rows ) ? count( $rows ) : 0,
            )
        );
        ...
        $can_manage_licences = ufsc_user_can( UFSC_Permissions::CAP_LICENCES_MANAGE );

        $normalized_filter_season = str_replace( '/', '-', '__current' === $filter_season ? $current_season_label : $filter_season );
        $historical_view = in_array( $filter_season, array( '__archives', 'all' ), true ) || ( preg_match( '/^\d{4}-\d{4}$/', $normalized_filter_season ) && $normalized_filter_season < $current_season_label );
        if ( $can_manage_licences ) {
            // Add nonce only when AJAX write operations are available.
            echo '<input type="hidden" id="ufsc-ajax-nonce" value="' . wp_create_nonce('ufsc_ajax_nonce') . '" />';
        }

        if ( $can_manage_licences ) {
            echo '<p><a href="' . esc_url(self::get_licences_admin_page_url( array( 'action' => 'new' ) )) . '" class="button button-primary">' . esc_html__('Ajouter une licence', 'ufsc-clubs') . '</a> ';
        ...
        echo '</select>';
		if ( $previous_season_label ) {
			$renewals_view_url = add_query_arg( array( 'filter_season' => $previous_season_label, 'filter_visibility' => 'active' ), $licences_page_url );
			echo ' <a class="button" href="' . esc_url( $renewals_view_url ) . '">' . esc_html__( 'Licences à renouveler', 'ufsc-clubs' ) . '</a>';
		}
        echo '</div>';
        ...
        if ( $can_manage_licences && ! $historical_view ) {
            ...
            // Stored historical rows are read-only; renewal is a per-row annual creation flow.
        ...
        echo '<th class="column-id">' . esc_html__('N° licence', 'ufsc-clubs') . '</th>';
        echo '<th>' . esc_html__('N° ASPTT', 'ufsc-clubs') . '</th>';
        ...
        if ($rows) {
            foreach ($rows as $r) {
                ...
                echo '<tr>';
                if ( $can_manage_licences && ! $historical_view ) {
                    echo $is_historical ? '<th class="check-column">—</th>' : '<th class="check-column"><input type="checkbox" name="licence_ids[]" value="' . $licence_id . '" /></th>';
				} elseif ( $can_manage_licences ) {
                ...
                echo '<td>' . esc_html( $asptt_number ) . '</td>';
                echo '<td><strong>' . esc_html( $r->nom ) . '</strong></td>';
				...
				if ( $is_historical ) {
					echo '<span class="ufsc-badge ufsc-badge-neutral">' . esc_html__( 'Saison terminée', 'ufsc-clubs' ) . '</span>';
					echo '<br><small>' . esc_html( sprintf( __( '%1$s en %2$s', 'ufsc-clubs' ), ufsc_get_licence_status_label_fr( $normalized_status ), $season_label ) ) . '</small>';
				} else {
                ...
                if ( $can_manage_licences && ! $is_historical ) {
                ...
                $is_paid = self::is_licence_paid( $r );
                if ( $is_historical ) {
					$action_label = (string) ( $season_context['action_label'] ?? '' );
					$action_url = (string) ( $season_context['action_url'] ?? '' );
					if ( ! $action_url && ! empty( $season_context['renewed_licence_id'] ) ) {
					...
					if ( ! empty( $season_context['renewal_allowed'] ) && function_exists( 'ufsc_get_licence_product_id' ) && ufsc_get_licence_product_id() ) {
						...
						echo '<span class="ufsc-badge ufsc-badge-neutral" title="' . esc_attr( $season_context['renewal_reason'] ?? '' ) . '">' . esc_html( $action_label ?: __( 'Renouvellement bloqué', 'ufsc-clubs' ) ) . '</span>';
						if ( ! empty( $season_context['renewal_reason'] ) ) { echo '<br><small>' . esc_html( $season_context['renewal_reason'] ) . '</small>'; }
					}
                } elseif ( $can_manage_licences && ! $is_paid ) {
                ...
                if ( ! $is_historical && $can_manage_licences && 'trash' === $filter_visibility ) {
                    echo '<a class="button button-small" href="' . esc_url( $restore_url ) . '" title="' . esc_attr__('Restaurer la licence', 'ufsc-clubs') . '">' . esc_html__('Restaurer', 'ufsc-clubs') . '</a>';
                    echo '<a class="button button-small button-link-delete" href="' . esc_url( $force_delete_url ) . '" title="' . esc_attr__('Supprimer définitivement la licence', 'ufsc-clubs') . '" onclick="return confirm(\'' . esc_js(__('Êtes-vous sûr de vouloir supprimer définitivement cette licence ?', 'ufsc-clubs')) . '\')">' . esc_html__('Supprimer définitivement', 'ufsc-clubs') . '</a>';
                } elseif ( ! $is_historical && $can_manage_licences && '' === $delete_block_reason ) {
                ...
                } elseif ( ! $is_historical && $can_manage_licences ) {
    ...
    private static function render_licence_form( $id, $readonly = false, $row = null, $admin_pk = '', $row_source = 'auto' )
    {
        ...
        self::render_licence_form_id_debug( $id, $row, $admin_pk, $row_source );
    ...
    public static function handle_save_licence()
    {
        ...
        // Vérifier droits sur le club pour les anciens flux rattachés à un club.
        if ( ! ufsc_user_can( UFSC_Permissions::CAP_LICENCES_MANAGE ) && ! ufsc_user_can( UFSC_Permissions::CAP_GESTION_MANAGE ) && ufsc_get_user_club_id($user_id) !== $club_id) {
            set_transient('ufsc_error_' . $user_id, __('Permissions insuffisantes', 'ufsc-clubs'), 30);
            self::maybe_redirect(wp_get_referer());
            return;
        }
    ...
    public static function handle_send_license_payment()
    {
        ...
        if (! $license_id) {
            self::maybe_redirect(self::get_licences_admin_page_url( array( 'error' => __( 'ID de licence invalide', 'ufsc-clubs' ) ) ));
            return;
        }
        global $wpdb;
        $s  = UFSC_SQL::get_settings();
        ...
        if ($order_id) {
            ...
            self::maybe_redirect(self::get_licences_admin_page_url( array( 'action' => 'edit', 'id' => $license_id, 'payment_sent' => 1, 'order_id' => $order_id ) ));
        } else {
            UFSC_CL_Utils::log('Erreur création commande pour licence ID ' . $license_id, 'error');
            self::maybe_redirect(self::get_licences_admin_page_url( array( 'action' => 'edit', 'id' => $license_id, 'error' => __( 'Erreur lors de la création de la commande de paiement', 'ufsc-clubs' ) ) ));
        }
        return;
    }
    ...
    public static function handle_delete_licence()
    {
        ...
        self::handle_trash_licence( false );
    }
    ...
    public static function handle_trash_licence( $check_nonce = true ) {
        ...
        if ( $check_nonce ) {
            check_admin_referer('ufsc_sql_trash_licence');
        }
        ...
        if ($id) {
            ...
            if ( '' !== $blocked_reason ) {
                self::maybe_redirect( self::build_licences_redirect_url( array( 'error' => urlencode( $blocked_reason ) ) ) );
                return;
            }
            ...
            if ($result !== false) {
                self::debug_log_licences( 'trash', array( 'licence_id' => (int) $id, 'actor' => get_current_user_id() ) );
                UFSC_CL_Utils::log('Licence en corbeille: ID ' . $id, 'info');
                self::maybe_redirect( self::build_licences_redirect_url( array( 'deleted' => 1, 'deleted_id' => $id ) ) );
            } else {
                UFSC_CL_Utils::log('Erreur corbeille licence: ID ' . $id, 'error');
                self::maybe_redirect( self::build_licences_redirect_url( array( 'error' => urlencode( __( 'Erreur lors de la mise en corbeille de la licence', 'ufsc-clubs' ) ) ) ) );
            }
        } else {
            self::maybe_redirect( self::build_licences_redirect_url( array( 'error' => urlencode( __( 'ID de licence invalide', 'ufsc-clubs' ) ) ) ) );
        }
    ...
    public static function handle_force_delete_licence() {
        ...
        $result = $wpdb->delete( $t, array( $pk => $id ), array( '%d' ) );
        self::debug_log_licences( 'force_delete', array( 'licence_id' => $id, 'actor' => get_current_user_id(), 'result' => $result ) );
        self::maybe_redirect( self::build_licences_redirect_url( array( 'deleted' => 1, 'deleted_id' => $id ) ) );
    }
    ...
    public static function handle_ajax_update_licence_status()
    {
        ...
        if (! wp_verify_nonce($_POST['nonce'], 'ufsc_ajax_nonce') || ! ufsc_user_can( UFSC_Permissions::CAP_LICENCES_MANAGE )) {
            wp_die();
        }
    ...
    public static function handle_export_data()
    {
        ...
        // Get filters
        $filters = self::get_licence_filters_from_request( 'post' );
        if ( empty( $filters['filter_visibility'] ) ) {
            $filters['filter_visibility'] = 'active';
        }
        $filter_club    = $filters['filter_club'];
        $filter_region  = $filters['filter_region'];
        $filter_status  = $filters['filter_status'];
    ...
    private static function bulk_restore_items($item_ids, $table, $pk) {
        ...
        add_action( 'admin_notices', function () use ( $restored ) {
            if ( $restored > 0 ) {
                echo '<div class="notice notice-success is-dismissible"><p>';
                printf(_n('%d élément restauré.', '%d éléments restaurés.', $restored), $restored);
                echo '</p></div>';
            }
        } );
    }
