...
class UFSC_Frontend_Shortcodes {
    ...
    /**
     * Return the canonical, single-fragment URL for the licence creation form.
     *
     * @param string $role   Optional licence role to preselect.
     * @param string $season Optional season to preserve in the journey.
     * @return string
     */
    public static function get_add_licence_url( $role = '', $season = '' ) {
        $role = sanitize_key( (string) $role );
        $allowed_roles = array( 'president', 'secretaire', 'tresorier', 'entraineur', 'coach', 'educateur', 'encadrant' );
        $args = array( 'ufsc_tab' => 'add_licence' );

        if ( in_array( $role, $allowed_roles, true ) ) {
            $args['ufsc_prefill_role'] = $role;
        }
        if ( '' !== trim( (string) $season ) ) {
            $args['ufsc_season'] = sanitize_text_field( $season );
        }

        // get_club_portal_url() has no fragment. Appending exactly one here is
        // essential: a double fragment leaves the browser on the licence list.
        return add_query_arg( $args, self::get_club_portal_url() ) . '#ufsc-section-add_licence';
    }
    ...
    public static function render_club_dashboard( $atts = array() ) {
        ...
        $dashboard_tab_urls = array();
        foreach ( array( 'licences', 'stats', 'profile', 'add_licence' ) as $dashboard_tab ) {
            $dashboard_tab_urls[ $dashboard_tab ] = add_query_arg( 'ufsc_tab', $dashboard_tab, self::get_club_portal_url() )
                . ( 'licences' === $dashboard_tab ? '#ufsc-club-licences' : '#ufsc-section-' . $dashboard_tab );
        }
                    ...
                    <div class="ufsc-hero-left">
                        ...
                        <div class="ufsc-dashboard-actions ufsc-dashboard-actions--primary">
                            <?php if ( in_array( 'add_licence', $sections, true ) ): ?>
                                <a href="<?php echo esc_url( self::get_add_licence_url( '', $current_season ) ); ?>" class="ufsc-btn ufsc-btn-primary">
                ...
                <div class="ufsc-dashboard-mainpane">
                    ...
                    <div class="ufsc-dashboard-nav">
                        <?php if ( in_array( 'stats', $sections ) ): ?>
                            <button type="button" class="ufsc-nav-btn<?php echo 'stats' === $requested_dashboard_section ? ' active' : ''; ?>" data-section="stats" data-url="<?php echo esc_url( $dashboard_tab_urls['stats'] ); ?>"<?php echo 'stats' === $requested_dashboard_section ? ' aria-current="page"' : ''; ?>><?php esc_html_e( 'Statistiques', 'ufsc-clubs' ); ?></button>
                        <?php endif; ?>
                        <?php if ( in_array( 'profile', $sections ) ): ?>
                            <button type="button" class="ufsc-nav-btn<?php echo 'profile' === $requested_dashboard_section ? ' active' : ''; ?>" data-section="profile" data-url="<?php echo esc_url( $dashboard_tab_urls['profile'] ); ?>"<?php echo 'profile' === $requested_dashboard_section ? ' aria-current="page"' : ''; ?>><?php esc_html_e( 'Mon Club', 'ufsc-clubs' ); ?></button>
                        <?php endif; ?>
                        <?php if ( in_array( 'add_licence', $sections ) ): ?>
                            <button type="button" class="ufsc-nav-btn<?php echo 'add_licence' === $requested_dashboard_section ? ' active' : ''; ?>" data-section="add_licence" data-url="<?php echo esc_url( $dashboard_tab_urls['add_licence'] ); ?>"<?php echo 'add_licence' === $requested_dashboard_section ? ' aria-current="page"' : ''; ?>><?php esc_html_e( 'Ajouter une Licence', 'ufsc-clubs' ); ?></button>
                        <?php endif; ?>
                        <?php if ( in_array( 'licences', $sections ) ): ?>
                            <button type="button" class="ufsc-nav-btn<?php echo 'licences' === $requested_dashboard_section ? ' active' : ''; ?>" data-section="licences" data-url="<?php echo esc_url( $dashboard_tab_urls['licences'] ); ?>"<?php echo 'licences' === $requested_dashboard_section ? ' aria-current="page"' : ''; ?>><?php esc_html_e( 'Mes licences UFSC', 'ufsc-clubs' ); ?></button>
                        <?php endif; ?>
                    </div>
                    <div class="ufsc-dashboard-content">
                        <?php if ( in_array( 'licences', $sections ) ): ?>
                            <div id="ufsc-club-licences" class="ufsc-dashboard-section<?php echo 'licences' === $requested_dashboard_section ? ' active' : ''; ?>">
                                <?php if ( 'licences' === $requested_dashboard_section ) { echo self::render_club_licences( array( 'club_id' => $club_id, 'readonly' => false ) ); } ?>
                            </div>
                        <?php endif; ?>

                        <?php if ( in_array( 'stats', $sections ) ): ?>
                            <div id="ufsc-section-stats" class="ufsc-dashboard-section<?php echo 'stats' === $requested_dashboard_section ? ' active' : ''; ?>">
                                <?php if ( 'stats' === $requested_dashboard_section ) { echo self::render_club_stats( array( 'club_id' => $club_id ) ); } ?>
                            </div>
                        <?php endif; ?>

                        <?php if ( in_array( 'profile', $sections ) ): ?>
                            <div id="ufsc-section-profile" class="ufsc-dashboard-section<?php echo 'profile' === $requested_dashboard_section ? ' active' : ''; ?>">
                                <?php if ( 'profile' === $requested_dashboard_section ) { echo self::render_club_profile( array( 'club_id' => $club_id ) ); } ?>
                            </div>
                        <?php endif; ?>

                        <?php if ( in_array( 'add_licence', $sections ) ): ?>
                            <div id="ufsc-section-add_licence" class="ufsc-dashboard-section<?php echo 'add_licence' === $requested_dashboard_section ? ' active' : ''; ?>">
                                <?php if ( 'add_licence' === $requested_dashboard_section ) { echo self::render_add_licence( array( 'club_id' => $club_id ) ); } ?>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
        ...
        jQuery(document).ready(function($) {
            ...
            dashboard.find('.ufsc-nav-btn').on('click.ufscDashboard', function() {
                ...
                // Only the active panel is rendered server-side. Loading the
                // selected route avoids generating three hidden, expensive
                // panels on every click.
                if (!$(this).hasClass('active') && targetUrl) {
    ...
    public static function render_club_licences( $atts = array() ) {
        ...
        $renew_per_page   = 10;
        $renew_total      = 0;
        $renewal_archives = array();

        // Building renewal candidates can inspect up to 5,000 historical
        // licences. Never do that work on the current-list or archive routes.
        if ( 'licences-renouvellement' === $requested_section ) {
            ...
            $renew_args = array_merge( $atts, $renew_filters, array( 'season' => $renewal_source_season ) );
            $all_renewable_sources = $renewal_source_season ? self::get_renewable_sources( $atts['club_id'], $renewal_source_season, $active_season, $renew_args ) : array();
            $renew_total = count( $all_renewable_sources );
            $renew_pages = max( 1, (int) ceil( $renew_total / $renew_per_page ) );
            $renew_page = min( $renew_page, $renew_pages );
            $renewal_archives = array_slice( $all_renewable_sources, ( $renew_page - 1 ) * $renew_per_page, $renew_per_page );
    ...
    public static function render_club_profile( $atts = array() ) {
        ...
        if (
            ...
            if ( $result['success'] ) {
                ...
                $club = self::get_club_data( $atts['club_id'], true ); // Refresh data
            } else {
    ...
    public static function render_add_licence( $atts = array() ) {
        ...
        if ( ! $is_edit_mode && empty( $form_data['role'] ) && isset( $_GET['ufsc_prefill_role'] ) && ! is_array( $_GET['ufsc_prefill_role'] ) ) {
            ...
            if ( in_array( $prefill_role, array( 'president', 'secretaire', 'tresorier', 'entraineur', 'coach', 'educateur', 'encadrant' ), true ) ) { $form_data['role'] = $prefill_role; }
        }
    ...
    /** Return the exact source rows advertised by the renewal KPI. */
    private static function get_renewable_sources( $club_id, $source_season, $target_season, $filters = array() ) {
        $args = array_merge( (array) $filters, array( 'season' => $source_season, 'status' => 'valide', 'page' => 1, 'per_page' => 5000 ) );
        unset( $args['renewal_state'] );
        $rows = self::get_club_licences( $club_id, $args );
        return array_values( array_filter( $rows, static function ( $row ) use ( $club_id, $target_season ) {
            if ( ! is_callable( array( 'UFSC_Renewal_Service', 'can_renew' ) ) ) { return false; }
            return true === UFSC_Renewal_Service::can_renew( $row, $club_id, $target_season );
        } ) );
    }
    ...
    private static function get_club_licences( $club_id, $args ) {
        ...
        // Statut
		$status_expression = in_array( 'statut', $columns, true ) && in_array( 'status', $columns, true )
			? "COALESCE(NULLIF(TRIM(`statut`), ''), `status`)"
			: ( in_array( 'statut', $columns, true ) ? '`statut`' : ( in_array( 'status', $columns, true ) ? '`status`' : '' ) );
        if ( ! empty( $args['status'] ) && $status_expression ) {
            ...
            if ( empty( $raw_values ) ) {
				...
				$clauses[]    = "{$status_expression} IN ({$placeholders})";
		...
		$has_demographic_filter = ! empty( $args['gender'] ) || ! empty( $args['practice'] ) || ! empty( $args['age'] ) || ! empty( $args['missing_profile'] ) || ! empty( $args['birth_from'] ) || ! empty( $args['birth_to'] );
		if ( $has_demographic_filter ) {
			if ( ! $status_expression ) { $clauses[] = '1 = 0'; }
			else {
				$official_values = function_exists( 'ufsc_get_licence_status_raw_values_for_norm' ) ? ufsc_get_licence_status_raw_values_for_norm( 'valide' ) : array( 'valide' );
				$placeholders = implode( ', ', array_fill( 0, count( $official_values ), '%s' ) );
				$clauses[] = "{$status_expression} IN ({$placeholders})";
				$values = array_merge( $values, $official_values );
			}
		}
    ...
    private static function get_bureau_coverage_data( $club_id, $season = '' ) {
        ...
        $data = array(
            'assignments' => array(
                ...
                'tresorier' => array(),
                'adherent' => array(),
                'entraineur' => array(), 'coach' => array(), 'educateur' => array(), 'encadrant' => array(),
            ),
            'licences' => array( 'president' => array(), 'secretaire' => array(), 'tresorier' => array(), 'coaches' => array() ),
        ...
        $cache_key = $club_id . '|' . (string) $season;
        if ( isset( $request_cache[ $cache_key ] ) ) {
            return $request_cache[ $cache_key ];
        }
        $where = "club_id = %d AND role IN ('president','secretaire','tresorier','adherent','entraineur','coach','educateur','encadrant')";
        ...
        foreach ( (array) $rows as $row ) {
            ...
            if ( in_array( $role, array( 'entraineur', 'coach', 'educateur', 'encadrant' ), true ) ) { $data['licences']['coaches'][] = $row; }
        }
    ...
    private static function render_officer_licence_card( $role, $bureau, $season ) {
        ...
        $create_url = self::get_add_licence_url( $role, $season );
        ?>
        <article class="ufsc-board-role-card" aria-labelledby="ufsc-officer-<?php echo esc_attr( $role ); ?>">
            ...
            <?php if ( $licence ) :
                ...
                $edit_url = add_query_arg( array( 'ufsc_tab' => 'add_licence', 'edit_licence' => $licence_id, 'ufsc_season' => $season ), self::get_club_portal_url() ) . '#ufsc-section-add_licence';
                ...
                $business_state = function_exists( 'ufsc_resolve_licence_business_state' ) ? ufsc_resolve_licence_business_state( $licence ) : array( 'payment' => 'non_recu' );
                $honorability = function_exists( 'ufsc_get_honorability_document' ) ? ufsc_get_honorability_document( $licence_id, $season ) : array( 'status' => 'missing' );
                $address = trim( implode( ', ', array_filter( array( $licence->adresse ?? '', $licence->suite_adresse ?? '', $licence->code_postal ?? '', $licence->ville ?? '', $licence->pays ?? '' ) ) ) );
                $missing = array();
                foreach ( array( 'nom', 'prenom', 'sexe', 'date_naissance', 'adresse', 'code_postal', 'ville', 'pays', 'email' ) as $field ) { if ( '' === trim( (string) ( $licence->{$field} ?? '' ) ) ) { $missing[] = $field; } }
                if ( '' === self::resolve_licence_phone( $licence ) ) { $missing[] = 'telephone'; }
            ?>
                <dl class="ufsc-officer-summary"><div><dt><?php esc_html_e( 'Identité', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( trim( (string) ( $licence->prenom ?? '' ) . ' ' . (string) ( $licence->nom ?? '' ) ) ); ?></dd></div><div><dt><?php esc_html_e( 'Rôle', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( $labels[ $role ] ); ?></dd></div><div><dt><?php esc_html_e( 'Sexe / naissance', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( trim( (string) ( $licence->sexe ?? '' ) . ' · ' . (string) ( $licence->date_naissance ?? '' ), ' ·' ) ?: '—' ); ?></dd></div><div><dt><?php esc_html_e( 'Adresse', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( $address ?: '—' ); ?></dd></div><div><dt><?php esc_html_e( 'Contact', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( trim( self::resolve_licence_phone( $licence ) . ' · ' . (string) ( $licence->email ?? '' ), ' ·' ) ?: '—' ); ?></dd></div><div><dt><?php esc_html_e( 'Pratique / niveau / poids', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( trim( (string) ( $licence->pratique ?? '' ) . ' · ' . (string) ( $licence->fighter_level ?? '' ) . ' · ' . (string) ( $licence->poids ?? '' ), ' ·' ) ?: '—' ); ?></dd></div><div><dt><?php esc_html_e( 'Saison', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( $season ); ?></dd></div><div><dt><?php esc_html_e( 'Dossier', 'ufsc-clubs' ); ?></dt><dd><?php echo self::get_status_badge_front( $status_raw ); ?></dd></div><div><dt><?php esc_html_e( 'Paiement', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( str_replace( '_', ' ', $business_state['payment'] ?? '' ) ); ?></dd></div><div><dt><?php esc_html_e( 'Pack', 'ufsc-clubs' ); ?></dt><dd><?php echo ! empty( $licence->is_included ) ? esc_html__( 'Incluse dans le pack', 'ufsc-clubs' ) : esc_html__( 'Licence supplémentaire payante', 'ufsc-clubs' ); ?></dd></div><div><dt><?php esc_html_e( 'Honorabilité', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( str_replace( '_', ' ', $honorability['status'] ?? 'missing' ) ); ?></dd></div><div><dt><?php esc_html_e( 'N° UFSC', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( $licence->numero_licence_ufsc ?? ( $licence->numero_licence ?? '—' ) ); ?></dd></div><div><dt><?php esc_html_e( 'Champs manquants', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( $missing ? implode( ', ', $missing ) : __( 'Aucun', 'ufsc-clubs' ) ); ?></dd></div></dl>
                <div class="ufsc-board-role-card__actions"><a class="ufsc-btn ufsc-btn-secondary" href="<?php echo esc_url( $detail_url ); ?>"><?php esc_html_e( 'Consulter', 'ufsc-clubs' ); ?></a><a class="ufsc-btn ufsc-btn-primary" href="<?php echo esc_url( $edit_url ); ?>"><?php esc_html_e( 'Modifier / compléter', 'ufsc-clubs' ); ?></a><?php if ( 'validated' !== ( $honorability['status'] ?? '' ) ) : ?><a class="ufsc-btn ufsc-btn-secondary" href="<?php echo esc_url( $edit_url . '#ufsc-honorability' ); ?>"><?php esc_html_e( 'Finaliser l’honorabilité', 'ufsc-clubs' ); ?></a><?php endif; ?><a class="ufsc-btn ufsc-btn-secondary" href="<?php echo esc_url( $renew_url ); ?>"><?php esc_html_e( 'Renouveler', 'ufsc-clubs' ); ?></a></div>
            <?php else : ?>
    ...
    /** Render every regulated coaching function from season-scoped licences. */
    private static function render_coach_licence_cards( $bureau, $season ) {
        $rows = array_values( (array) ( $bureau['licences']['coaches'] ?? array() ) );
        $create_url = self::get_add_licence_url( 'entraineur', $season );
        ?>
        <section class="ufsc-coach-licences" aria-labelledby="ufsc-coach-licences-title">
            <div class="ufsc-pack-summary__heading">
                <h5 id="ufsc-coach-licences-title"><?php esc_html_e( 'Entraîneurs et encadrants', 'ufsc-clubs' ); ?></h5>
                <a class="ufsc-btn ufsc-btn-primary" href="<?php echo esc_url( $create_url ); ?>"><?php esc_html_e( 'Ajouter un entraîneur', 'ufsc-clubs' ); ?></a>
            </div>
            <?php if ( ! $rows ) : ?>
                <p class="ufsc-message ufsc-info"><?php esc_html_e( 'Aucune licence d’entraîneur ou d’encadrant pour cette saison.', 'ufsc-clubs' ); ?></p>
            <?php else : ?>
                <div class="ufsc-board-columns">
                <?php foreach ( $rows as $licence ) :
                    $licence_id = absint( $licence->id ?? 0 );
                    $role = sanitize_key( (string) ( $licence->role ?? 'entraineur' ) );
                    $state = function_exists( 'ufsc_resolve_licence_business_state' ) ? ufsc_resolve_licence_business_state( $licence ) : array( 'dossier' => ufsc_get_licence_status_from_record( $licence ), 'payment' => 'non_recu' );
                    $required = array( 'nom', 'prenom', 'date_naissance', 'sexe', 'adresse', 'code_postal', 'ville', 'pays', 'email' );
                    $missing = array();
                    foreach ( $required as $field ) { if ( '' === trim( (string) ( $licence->{$field} ?? '' ) ) ) { $missing[] = $field; } }
                    if ( '' === self::resolve_licence_phone( $licence ) ) { $missing[] = 'telephone'; }
                    $honorability = function_exists( 'ufsc_get_honorability_document' ) ? ufsc_get_honorability_document( $licence_id, $season ) : array( 'status' => 'missing' );
                    $detail_url = self::get_licence_detail_url( $licence_id );
                    $edit_url = add_query_arg( array( 'ufsc_tab' => 'add_licence', 'edit_licence' => $licence_id, 'ufsc_season' => $season ), self::get_club_portal_url() ) . '#ufsc-section-add_licence';
                    ?>
                    <article class="ufsc-board-role-card" aria-labelledby="ufsc-coach-<?php echo esc_attr( $licence_id ); ?>">
                        <div class="ufsc-board-role-card__heading"><h6 id="ufsc-coach-<?php echo esc_attr( $licence_id ); ?>"><?php echo esc_html( trim( (string) ( $licence->prenom ?? '' ) . ' ' . (string) ( $licence->nom ?? '' ) ) ); ?></h6><span class="ufsc-badge ufsc-badge-info"><?php echo esc_html( ucfirst( str_replace( '_', ' ', $role ) ) ); ?></span></div>
                        <dl class="ufsc-officer-summary">
                            <div><dt><?php esc_html_e( 'Saison', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( $season ); ?></dd></div>
                            <div><dt><?php esc_html_e( 'Dossier', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( str_replace( '_', ' ', $state['dossier'] ?? '' ) ); ?></dd></div>
                            <div><dt><?php esc_html_e( 'Paiement', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( str_replace( '_', ' ', $state['payment'] ?? '' ) ); ?></dd></div>
                            <div><dt><?php esc_html_e( 'Pack', 'ufsc-clubs' ); ?></dt><dd><?php echo ! empty( $licence->is_included ) ? esc_html__( 'Incluse dans le pack', 'ufsc-clubs' ) : esc_html__( 'Licence supplémentaire payante', 'ufsc-clubs' ); ?></dd></div>
                            <div><dt><?php esc_html_e( 'Téléphone', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( self::resolve_licence_phone( $licence ) ?: '—' ); ?></dd></div>
                            <div><dt><?php esc_html_e( 'E-mail', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( $licence->email ?? '—' ); ?></dd></div>
                            <div><dt><?php esc_html_e( 'Pratique / niveau', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( trim( (string) ( $licence->pratique ?? '' ) . ' ' . (string) ( $licence->fighter_level ?? '' ) ) ?: '—' ); ?></dd></div>
                            <div><dt><?php esc_html_e( 'Honorabilité', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( str_replace( '_', ' ', $honorability['status'] ?? 'missing' ) ); ?></dd></div>
                            <div><dt><?php esc_html_e( 'Champs manquants', 'ufsc-clubs' ); ?></dt><dd><?php echo esc_html( $missing ? implode( ', ', $missing ) : __( 'Aucun', 'ufsc-clubs' ) ); ?></dd></div>
                        </dl>
                        <div class="ufsc-board-role-card__actions"><a class="ufsc-btn ufsc-btn-secondary" href="<?php echo esc_url( $detail_url ); ?>"><?php esc_html_e( 'Consulter', 'ufsc-clubs' ); ?></a><a class="ufsc-btn ufsc-btn-primary" href="<?php echo esc_url( $edit_url ); ?>"><?php esc_html_e( 'Modifier / compléter', 'ufsc-clubs' ); ?></a><?php if ( 'validated' !== ( $honorability['status'] ?? '' ) ) : ?><a class="ufsc-btn ufsc-btn-secondary" href="<?php echo esc_url( $edit_url . '#ufsc-honorability' ); ?>"><?php esc_html_e( 'Finaliser l’honorabilité', 'ufsc-clubs' ); ?></a><?php endif; ?></div>
                    </article>
                <?php endforeach; ?>
                </div>
            <?php endif; ?>
    ...
    private static function get_club_stats( $club_id, $season ) {
        static $request_cache = array();
        $cache_key = (int) $club_id . '|' . (string) $season;
     ...
     * Get club data
     */
    private static function get_club_data( $club_id, $force = false ) {
        global $wpdb;

        static $request_cache = array();
        $club_id = (int) $club_id;
        if ( ! $force && isset( $request_cache[ $club_id ] ) ) {
            return $request_cache[ $club_id ];
        }
        ...
        $request_cache[ $club_id ] = $club ?: (object) array(
            'id'        => (int) $club_id,
            'nom'       => 'Club',
            'email'     => '',
            'telephone' => '',
     ...
     */
    private static function render_ffst_club_profile_section( $club, $is_admin = false, $annual_status = '' ) {
        ...
        $required_fields = array(
            ...
            'president_adresse',
            'president_code_postal',
            'president_ville',
            'secretaire_nom',
            'secretaire_prenom',
            'secretaire_date_naissance',
            'secretaire_adresse',
            'secretaire_code_postal',
            'secretaire_ville',
            'tresorier_nom',
            'tresorier_prenom',
            'tresorier_date_naissance',
            'tresorier_adresse',
        ...
        foreach ( $required_fields as $field ) {
            if ( '' === trim( $value( $field ) ) ) { $missing[] = $field; }
        }
        $total = count( $required_fields );
        $done = $total - count( $missing );
        $percent = $total ? (int) round( ( $done / $total ) * 100 ) : 100;

        $active_statuses = array( 'active', 'actif', 'valide', 'validated', 'approved' );
        $is_active = in_array( sanitize_key( (string) $annual_status ), $active_statuses, true );
        ...
        ?>
        <section class="ufsc-card ufsc-form-section ufsc-club-portal__section--full" id="ufsc-club-ffst">
            <div class="ufsc-section-header">
                <div>
                    <h4><?php esc_html_e( 'Affiliation & informations FFST', 'ufsc-clubs' ); ?></h4>
                    <p class="ufsc-admin-help">
                        <?php
                        if ( $is_active && $missing ) {
                            esc_html_e( 'Votre affiliation actuelle reste active. Les informations marquées d’un * devront être complétées avant la prochaine affiliation ou réaffiliation.', 'ufsc-clubs' );
                        } elseif ( $missing ) {
                            esc_html_e( 'Les informations marquées d’un * sont obligatoires pour finaliser une prochaine affiliation FFST. Vous pouvez enregistrer votre progression sans bloquer votre compte.', 'ufsc-clubs' );
                        } else {
                            esc_html_e( 'Le dossier FFST contient toutes les informations de référence actuellement attendues.', 'ufsc-clubs' );
                        }
                        ?>
                    </p>
                </div>
                <div class="ufsc-profile-insight">
                    <strong class="ufsc-profile-insight__value"><?php echo esc_html( $percent ); ?> %</strong>
                    <span class="ufsc-profile-insight__label"><?php echo esc_html( sprintf( __( '%1$d / %2$d informations', 'ufsc-clubs' ), $done, $total ) ); ?></span>
                </div>
            </div>

            <?php if ( $missing ) : ?>
                <div class="ufsc-message ufsc-info">
                    <strong><?php echo esc_html( sprintf( _n( '%d information reste à compléter.', '%d informations restent à compléter.', count( $missing ), 'ufsc-clubs' ), count( $missing ) ) ); ?></strong>
                    <?php if ( $is_active ) : ?>
                        <?php esc_html_e( ' Cela ne désactive pas l’affiliation en cours.', 'ufsc-clubs' ); ?>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
            ...
            <div class="ufsc-grid">
                <div class="ufsc-card ufsc-form-section">
                    <h5><?php esc_html_e( 'Références fédérales', 'ufsc-clubs' ); ?></h5>
                    <div class="ufsc-grid">
                        <?php self::render_ffst_profile_input( $club, 'numero_affiliation_ffst', __( 'N° affiliation FFST', 'ufsc-clubs' ), 'text', ! $is_admin, __( 'Attribué par la FFST.', 'ufsc-clubs' ), false ); ?>
                        <?php self::render_ffst_profile_input( $club, 'disciplines_ffst', __( 'Disciplines FFST pratiquées', 'ufsc-clubs' ), 'text', false, __( 'Séparez plusieurs disciplines par une virgule.', 'ufsc-clubs' ), true ); ?>
                        <?php self::render_ffst_profile_input( $club, 'codes_disciplines_ffst', __( 'Codes disciplines FFST', 'ufsc-clubs' ), 'text', false, __( 'Codes correspondant aux disciplines.', 'ufsc-clubs' ), false ); ?>
                ...
                <div class="ufsc-card ufsc-form-section">
                    <h5><?php esc_html_e( 'Lieu principal d’entraînement', 'ufsc-clubs' ); ?></h5>
                    <div class="ufsc-grid">
                        <?php self::render_ffst_profile_input( $club, 'adresse_salle', __( 'Adresse de la salle', 'ufsc-clubs' ), 'text', false, '', true ); ?>
                        <?php self::render_ffst_profile_input( $club, 'complement_adresse_salle', __( 'Complément d’adresse', 'ufsc-clubs' ) ); ?>
                        <?php self::render_ffst_profile_input( $club, 'code_postal_salle', __( 'Code postal', 'ufsc-clubs' ), 'text', false, '', true ); ?>
                        <?php self::render_ffst_profile_input( $club, 'ville_salle', __( 'Ville', 'ufsc-clubs' ), 'text', false, '', true ); ?>
                    </div>
                </div>
            </div>
            ...
            <div class="ufsc-grid">
                <div class="ufsc-card ufsc-form-section">
                    <h5><?php esc_html_e( 'Correspondant FFST', 'ufsc-clubs' ); ?></h5>
                    <p class="ufsc-admin-help"><?php esc_html_e( 'Contact privilégié pour les échanges fédéraux.', 'ufsc-clubs' ); ?></p>
                    <div class="ufsc-grid">
                        <?php self::render_ffst_profile_input( $club, 'correspondant_nom', __( 'Nom', 'ufsc-clubs' ), 'text', false, '', true ); ?>
                        <?php self::render_ffst_profile_input( $club, 'correspondant_prenom', __( 'Prénom', 'ufsc-clubs' ), 'text', false, '', true ); ?>
                        <?php self::render_ffst_profile_input( $club, 'correspondant_tel', __( 'Téléphone', 'ufsc-clubs' ), 'tel', false, '', true ); ?>
                        <?php self::render_ffst_profile_input( $club, 'correspondant_email', __( 'E-mail', 'ufsc-clubs' ), 'email', false, '', true ); ?>
                    </div>
                </div>

                <div class="ufsc-card ufsc-form-section">
                    <h5><?php esc_html_e( 'Signataire du dossier', 'ufsc-clubs' ); ?></h5>
                    <p class="ufsc-admin-help"><?php esc_html_e( 'Personne qui signe les documents d’affiliation.', 'ufsc-clubs' ); ?></p>
                    <div class="ufsc-grid">
                        <?php self::render_ffst_profile_input( $club, 'signataire_nom', __( 'Nom', 'ufsc-clubs' ), 'text', false, '', true ); ?>
                        <?php self::render_ffst_profile_input( $club, 'signataire_prenom', __( 'Prénom', 'ufsc-clubs' ), 'text', false, '', true ); ?>
                        <?php self::render_ffst_profile_input( $club, 'signataire_qualite', __( 'Qualité', 'ufsc-clubs' ), 'text', false, __( 'Ex. Président, secrétaire général.', 'ufsc-clubs' ), true ); ?>
                    </div>
                </div>
            </div>
            ...
            <div class="ufsc-card ufsc-form-section ufsc-club-portal__section--full">
                <div class="ufsc-section-header">
                    <div>
                        <h5><?php esc_html_e( 'Bureau & encadrement', 'ufsc-clubs' ); ?></h5>
                        <p class="ufsc-admin-help"><?php esc_html_e( 'Les identités déjà enregistrées sont conservées. Les champs marqués d’un * sont requis pour le prochain dossier FFST.', 'ufsc-clubs' ); ?></p>
                    </div>
                    <a class="ufsc-btn ufsc-btn-secondary" href="<?php echo esc_url( self::get_club_portal_url( 'club-officers' ) ); ?>"><?php esc_html_e( 'Gérer les dirigeants', 'ufsc-clubs' ); ?></a>
                </div>

                <div class="ufsc-board-columns">
                    <?php foreach ( array( 'president', 'secretaire', 'tresorier' ) as $prefix ) :
                        $role_label = $leaders[ $prefix ];
                        $name = trim( $value( $prefix . '_prenom' ) . ' ' . $value( $prefix . '_nom' ) );
                    ?>
                        <article class="ufsc-card ufsc-form-section">
                            <h5><?php echo esc_html( $role_label . ( $name ? ' — ' . $name : '' ) ); ?></h5>
                            <div class="ufsc-grid">
                                <?php self::render_ffst_profile_input( $club, $prefix . '_date_naissance', __( 'Date de naissance', 'ufsc-clubs' ), 'date', false, '', true ); ?>
                                ...
                                <?php self::render_ffst_profile_input( $club, $prefix . '_ville', __( 'Ville', 'ufsc-clubs' ), 'text', false, '', true ); ?>
                            </div>
                        </article>
                    <?php endforeach; ?>
                </div>
                ...
                <?php
                    $prefix = 'entraineur';
                    $name = trim( $value( 'entraineur_prenom' ) . ' ' . $value( 'entraineur_nom' ) );
                ?>
                <article class="ufsc-card ufsc-form-section">
                    <h5><?php echo esc_html( $leaders['entraineur'] . ( $name ? ' — ' . $name : '' ) ); ?></h5>
                    <div class="ufsc-grid">
                        <?php self::render_ffst_profile_input( $club, 'entraineur_date_naissance', __( 'Date de naissance', 'ufsc-clubs' ), 'date' ); ?>
                        ...
                        <?php self::render_ffst_profile_input( $club, 'entraineur_ville', __( 'Ville', 'ufsc-clubs' ) ); ?>
                    </div>
                </article>
            </div>
        </section>
    ...
    private static function render_ffst_profile_input( $club, $field, $label, $type = 'text', $readonly = false, $help = '', $required_for_affiliation = false ) {
        ...
        } elseif ( $required_for_affiliation ) {
            echo '<small>' . esc_html__( 'Obligatoire avant la prochaine affiliation ou réaffiliation.', 'ufsc-clubs' ) . '</small>';
        }
        echo '<span class="ufsc-field-error" aria-live="polite"></span></div>';
    }
